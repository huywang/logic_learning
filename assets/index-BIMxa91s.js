...
`;const k=i("#weekTabs"),b=i("#lessonList"),I=i("#lessonContent"),O=i("#progressCount"),q=i("#progressBar"),u=i("#initiativeRange"),S=i("#initiativeValue"),g=i("#persistenceRange"),$=i("#persistenceValue"),h=i("#transferRange"),L=i("#transferValue"),f=i("#notesInput"),C=i("#completeBtn");B();x();R();U();function B(){k.innerHTML=Array.from({length:12},(e,n)=>{const s=n+1;return`<button class="${t.week===s?"active":""}" data-week="${s}" type="button">第 ${s} 周</button>`}).join(""),k.querySelectorAll("button").forEach(e=>{e.addEventListener("click",()=>{const n=Number(e.dataset.week);A(r.find(s=>s.week===n)??r[0])})})}function x(){b.innerHTML=r.filter(e=>e.week===t.week).map(e=>{const n=e.id===t.id?"active":"",s=o.completed.includes(e.id)?"done":"";return`
        <button class="lesson-item ${n} ${s}" data-lesson="${e.id}" type="button">
          <span class="lesson-number">${e.id}</span>
          <span>
            <strong>${e.title}</strong>
            <small>${e.type} · ${e.duration}</small>
          </span>
        </button>
      `}).join(""),b.querySelectorAll("button").forEach(e=>{e.addEventListener("click",()=>{const n=r.find(s=>s.id===Number(e.dataset.lesson));n&&A(n)})})}function R(){p=0,d=!1,o.selectedLessonId=t.id,c(),E(),I.innerHTML=`
    <div class="lesson-hero">
      <div>
        <span class="type-pill ${V[t.type]}">${t.type}</span>
        <h2>第 ${t.id} 课：${t.title}</h2>
        <p>${t.goal}</p>
      </div>
      <div class="meta-grid">
        <span>第 ${t.week} 周</span>
    ...
    <article class="teaching-block">
      ...
      ${m(t.prompt,"prompt-list")}
    </article>
    <article class="teaching-block">
      <div class="block-head"><span>解决引导</span></div>
      ${m(t.guide,"guide-list")}
    </article>
    <article class="teaching-block">
    ...
    <article class="teaching-block">
      ...
      <div id="solutionBox" class="solution-box hidden-content">${m(t.solution,"solution-list")}</div>
    </article>
  ...
  `,i("#hintBtn").addEventListener("click",H),i("#solutionBtn").addEventListener("click",M),T()}function w(e,n){return`
    <article class="mini-card">
      <h3>${e}</h3>
      ${m(n,"mini-list")}
    </article>
  `}function m(e,n){return`<ul class="${n}">${e.map(s=>`<li>${s}</li>`).join("")}</ul>`}function H(){const e=i("#hintList"),n=i("#hintBtn");if(p<t.hints.length){const s=document.createElement("li");s.textContent=t.hints[p],e.appendChild(s),p+=1}p>=t.hints.length&&(n.textContent="提示已全部显示",n.disabled=!0)}function M(){const e=i("#solutionBox"),n=i("#solutionBtn");d=!d,e.classList.toggle("hidden-content",!d),n.textContent=d?"隐藏解法":"显示解法"}function A(e){t=e,B(),x(),R()}function U(){u.addEventListener("input",()=>{o.initiative[t.id]=Number(u.value),S.textContent=`${u.value} 分`,c()}),g.addEventListener("input",()=>{o.persistence[t.id]=Number(g.value),$.textContent=`${g.value} 分`,c()}),h.addEventListener("input",()=>{o.transfer[t.id]=Number(h.value),L.textContent=`${h.value} 分`,c()}),f.addEventListener("input",()=>{o.notes[t.id]=f.value,c()}),C.addEventListener("click",()=>{const e=new Set(o.completed);e.has(t.id)?e.delete(t.id):e.add(t.id),o.completed=[...e].sort((n,s)=>n-s),c(),x(),T(),E()})}function T(){u.value=String(o.initiative[t.id]??3),g.value=String(o.persistence[t.id]??3),h.value=String(o.transfer[t.id]??3),S.textContent=`${u.value} 分`,$.textContent=`${g.value} 分`,L.textContent=`${h.value} 分`,f.value=o.notes[t.id]??"",C.textContent=o.completed.includes(t.id)?"标记为未完成":"标记本课完成"}function E(){const e=o.completed.length;O.textContent=`${e}/${r.length}`,q.style.width=`${e/r.length*100}%`}function P(){const e={selectedLessonId:1,completed:[],notes:{},initiative:{},persistence:{},transfer:{}};try{const n=localStorage.getItem("logic-course-progress-advanced");return n?{...e,...JSON.parse(n)}:e}catch{return e}}function c(){localStorage.setItem("logic-course-progress-advanced",JSON.stringify(o))}function i(e){const n=document.querySelector(e);if(!n)throw new Error(`Missing element: ${e}`);return n}
